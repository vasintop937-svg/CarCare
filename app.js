...
(function(){ var s=document.getElementById('preload-status'); if(s) s.textContent='Firebase OK ✓'; })();
...
(function(){ var s=document.getElementById('preload-status'); if(s) s.textContent='Тексты OK ✓'; })();
...
function render() {
  ...
  try {
    ...
    app.innerHTML = '<div style="padding:20px;color:#EF4444;font-size:13px;font-family:monospace;word-break:break-all"><b>Ошибка render():</b><br>' + e.message + '<br>' + (e.stack||'') + '</div>';
  }
}
...
function buildCar() {
  ...
  const tripBtn = on(mk('button',{css:'width:100%;margin-top:8px;padding:10px;border:1px solid rgba(255,255,255,.1);border-radius:12px;background:#1E2230;color:#F0F2F8;font-family:Syne,sans-serif;font-size:13px;font-weight:600;cursor:pointer',txt:'🗺️ Подготовка к поездке'}), 'click', function(){ openTripModal(car); });
  hero.appendChild(tripBtn);
    ...
    try {
      for (let i=0;i<D.records.length;i++) {
        if (D.records[i].id===rec.id) {
          ...
          if (rec.category==='fuel' && typeof priceInp!=='undefined') {
            D.records[i].pricePerLiter = priceInp.value;
            D.records[i].liters = litersInp.value;
            D.records[i].station = stationInp.value.trim();
          }
          if (rec.category==='insurance' && typeof expiryInp!=='undefined') {
            D.records[i].expiry = expiryInp.value;
          }
          break;
        }
      }
      saveNow(D); closeModal(); showToast('\u0417\u0430\u043f\u0438\u0441\u044c \u043e\u0431\u043d\u043e\u0432\u043b\u0435\u043d\u0430 \u2713'); render();
    } catch(e) {
...
function buildRecForm(modal) {
  ...
  function buildDyn(cat) {
    ...
    if (cat==='maintenance') {
      ...
      // Список деталей
      const partsField = mk('div',{cls:'field'});
      partsField.appendChild(mk('label',{txt:'\u0414\u0435\u0442\u0430\u043b\u0438 / \u0437\u0430\u043f\u0447\u0430\u0441\u0442\u0438'}));
      const partsList = mk('div',{css:'display:flex;flex-direction:column;gap:6px;margin-bottom:6px'});
      formData.parts = [];

      function addPartRow(val) {
        const row = mk('div',{css:'display:flex;gap:6px;align-items:center'});
        const inp = mkInp('text',''); inp.placeholder='\u041d\u0430\u0437\u0432\u0430\u043d\u0438\u0435 \u0434\u0435\u0442\u0430\u043b\u0438'; inp.value=val||'';
        inp.style.cssText='flex:1';
        const del = mk('button',{css:'min-width:36px;height:40px;border-radius:10px;border:1px solid rgba(239,68,68,.3);background:rgba(239,68,68,.1);color:#EF4444;cursor:pointer;font-size:16px;flex-shrink:0',txt:'\u00d7'});
        on(del,'click',function(){
          partsList.removeChild(row);
          formData.parts = formData.parts.filter(function(p){return p!==inp;});
        });
        row.appendChild(inp); row.appendChild(del);
        partsList.appendChild(row);
        formData.parts.push(inp);
      }

      addPartRow('');
      const addPartBtn = mk('button',{cls:'pbtn',css:'margin-top:4px;width:100%',txt:'+ \u0414\u043e\u0431\u0430\u0432\u0438\u0442\u044c \u0434\u0435\u0442\u0430\u043b\u044c'});
      on(addPartBtn,'click',function(){ addPartRow(''); });
      partsField.appendChild(partsList);
      partsField.appendChild(addPartBtn);
      dynForm.appendChild(partsField);
  ...
  function saveRecord(cat) {
    ...
    if (cat==='maintenance') {
      ...
      const partVals = (formData.parts||[]).map(function(p){return p.value.trim();}).filter(function(p){return p;});
      rec.part = partVals.join(', ');
      rec.parts = partVals;
...
function renderAuth(tab) {
  ...
  app.appendChild(wrap);
    ...
    app.innerHTML = '<div style="padding:20px;color:#EF4444;font-size:13px;font-family:monospace;word-break:break-all"><b>Ошибка renderAuth():</b><br>' + e.message + '<br>' + (e.stack||'') + '</div>';
  }
}
...
const YAGPT_KEY = null; // Ключ хранится в профиле пользователя (D.settings.yagptKey)
const YAGPT_FOLDER = 'b1gke3ef7sc9lvcosok4';

function getYagptKey() {
  return (D.settings && D.settings.yagptKey) ? D.settings.yagptKey : null;
}

function openTripModal(car) {
  const recs = D.records.filter(function(r){ return r.carId === car.id; });
  const reminders = (D.reminders||[]).filter(function(r){ return r.carId === car.id; });

  closeModal();
  const ov = mk('div',{cls:'overlay'});
  on(ov,'click',function(e){ if(e.target===ov) closeModal(); });

  const modal = mk('div',{cls:'modal'});
  modal.appendChild(mk('div',{cls:'mhdr'},[
    mk('div',{cls:'mtitle',txt:'🗺️ Подготовка к поездке'}),
    on(mk('button',{cls:'ibtn',txt:'✕'}),'click',closeModal)
  ]));

  // Поля маршрута
  const fromInp = mk('input',{cls:'inp',type:'text',placeholder:'Откуда (напр. Москва)'});
  const toInp = mk('input',{cls:'inp',type:'text',placeholder:'Куда (напр. Симферополь)'});
  const distInp = mk('input',{cls:'inp',type:'number',placeholder:'Примерное расстояние (км)'});

  modal.appendChild(mk('div',{cls:'field'},[mk('label',{txt:'ОТКУДА'}),fromInp]));
  modal.appendChild(mk('div',{cls:'field'},[mk('label',{txt:'КУДА'}),toInp]));
  modal.appendChild(mk('div',{cls:'field'},[mk('label',{txt:'РАССТОЯНИЕ (КМ)'}),distInp]));

  // Результаты
  const resultBox = mk('div',{css:'margin-top:16px'});
  modal.appendChild(resultBox);

  // Кнопка анализ
  const analyzeBtn = on(mk('button',{cls:'btnok',txt:'🤖 Анализировать поездку'}),'click',function(){
    const from = fromInp.value.trim();
    const to = toInp.value.trim();
    const dist = parseInt(distInp.value)||0;
    if (!from||!to) { showToast('Укажи откуда и куда'); return; }

    analyzeBtn.textContent = '⏳ Анализируем...';
    analyzeBtn.disabled = true;
    resultBox.innerHTML = '';

    runTripAnalysis(car, recs, reminders, from, to, dist, resultBox, function(){
...
function runTripAnalysis(car, recs, reminders, from, to, dist, box, done) {
  // Формируем историю для ИИ
  const lastRecs = recs.slice().sort(function(a,b){ return (b.mileage||0)-(a.mileage||0); }).slice(0,20);
  const historyText = lastRecs.map(function(r){
    return r.date+' | '+r.mileage+' км | '+r.title+(r.part?' ('+r.part+')':'')+(r.cost?' | '+r.cost+'₽':'');
  }).join('\n');

  const remText = reminders.map(function(r){
    const km = parseInt(r.currentKm||car.mileage||0);
    const lastKm = parseInt(r.lastKm||0);
    const intKm = parseInt(r.intervalKm||0);
    const left = intKm ? (lastKm + intKm - km) : null;
    return r.name + (left!==null?' (осталось ~'+left+' км)':'');
  }).join(', ');

  const prompt = `Ты автомеханик-эксперт. Анализируй данные автомобиля и дай рекомендации для поездки.

Автомобиль: ${car.make||''} ${car.model||''} ${car.year||''}, двигатель: ${car.engine||'не указан'}, пробег: ${car.mileage||0} км.
Маршрут: ${from} → ${to}, расстояние: ${dist||'неизвестно'} км.

История обслуживания (последние записи):
${historyText||'Записей нет'}

Напоминания: ${remText||'нет'}

Ответь строго в формате JSON (без markdown, без пояснений вне JSON):
{
  "checklist": ["пункт 1", "пункт 2", ...],
  "risks": [{"item": "название", "level": "высокий/средний/низкий", "comment": "пояснение"}],
  "fuel": {"liters": число, "cost": число, "comment": "пояснение"},
  ...
  .then(function(data){
    var text = data.result && data.result.alternatives && data.result.alternatives[0]
      ? data.result.alternatives[0].message.text : '';
    // Убираем возможные markdown-обёртки
    text = text.replace(/```json|```/g,'').trim();
    var parsed;
    try { parsed = JSON.parse(text); } catch(e) { parsed = null; }
    renderTripResult(box, parsed, text, from, to, dist, done);
  })
  .catch(function(e){
    box.innerHTML = '<div style="color:#EF4444;font-size:13px;padding:12px;background:rgba(239,68,68,.1);border-radius:12px">Ошибка запроса к ИИ: '+e.message+'</div>';
    done();
  });
}

function renderTripResult(box, data, rawText, from, to, dist, done) {
  box.innerHTML = '';

  if (!data) {
    box.appendChild(mk('div',{css:'background:#1E2230;border-radius:12px;padding:16px;font-size:13px;color:#F0F2F8;white-space:pre-wrap',txt:rawText}));
    done(); return;
  }

  // Итог
  if (data.summary) {
    box.appendChild(mk('div',{css:'background:rgba(200,255,0,.08);border:1px solid rgba(200,255,0,.2);border-radius:14px;padding:14px;margin-bottom:12px;font-size:14px;line-height:1.5;color:#F0F2F8',txt:'💡 '+data.summary}));
  }

  // Чеклист
  if (data.checklist && data.checklist.length) {
    const cl = mk('div',{css:'background:#1E2230;border-radius:14px;padding:16px;margin-bottom:12px'});
    cl.appendChild(mk('div',{css:'font-family:Syne,sans-serif;font-size:14px;font-weight:700;margin-bottom:10px;color:#F0F2F8',txt:'✅ Что проверить перед выездом'}));
    data.checklist.forEach(function(item){
      const row = mk('div',{css:'display:flex;gap:10px;align-items:flex-start;padding:6px 0;border-bottom:1px solid rgba(255,255,255,.05)'});
      const cb = mk('input',{type:'checkbox',css:'margin-top:2px;flex-shrink:0;accent-color:#C8FF00;width:16px;height:16px'});
      const lbl = mk('div',{css:'font-size:13px;color:#F0F2F8;line-height:1.4',txt:item});
      on(cb,'change',function(){
        lbl.style.textDecoration = cb.checked ? 'line-through' : 'none';
        lbl.style.color = cb.checked ? '#7A8099' : '#F0F2F8';
      });
  ...
  if (data.risks && data.risks.length) {
    const rk = mk('div',{css:'background:#1E2230;border-radius:14px;padding:16px;margin-bottom:12px'});
    rk.appendChild(mk('div',{css:'font-family:Syne,sans-serif;font-size:14px;font-weight:700;margin-bottom:10px;color:#F0F2F8',txt:'⚠️ Риски'}));
    data.risks.forEach(function(r){
      var color = r.level==='высокий'?'#EF4444':r.level==='средний'?'#F59E0B':'#10B981';
      var bg = r.level==='высокий'?'rgba(239,68,68,.08)':r.level==='средний'?'rgba(245,158,11,.08)':'rgba(16,185,129,.08)';
      const row = mk('div',{css:'padding:10px;border-radius:10px;margin-bottom:6px;border:1px solid '+color+'33;background:'+bg});
      row.appendChild(mk('div',{css:'display:flex;justify-content:space-between;align-items:center;margin-bottom:4px'},[
        mk('div',{css:'font-size:13px;font-weight:600;color:#F0F2F8',txt:r.item||''}),
        mk('div',{css:'font-size:11px;font-weight:600;color:'+color,txt:r.level||''})
      ]));
      if (r.comment) row.appendChild(mk('div',{css:'font-size:12px;color:#949AB5',txt:r.comment}));
      rk.appendChild(row);
    });
    box.appendChild(rk);
  }
  ...
  // Топливо
  if (data.fuel) {
    const fu = mk('div',{css:'background:#1E2230;border-radius:14px;padding:16px;margin-bottom:12px'});
    fu.appendChild(mk('div',{css:'font-family:Syne,sans-serif;font-size:14px;font-weight:700;margin-bottom:10px;color:#F0F2F8',txt:'⛽ Топливо'}));
    const grid = mk('div',{css:'display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:8px'});
    if (data.fuel.liters) {
      const l = mk('div',{css:'background:rgba(255,255,255,.04);border-radius:10px;padding:12px'});
      l.appendChild(mk('div',{css:'font-family:Syne,sans-serif;font-size:20px;font-weight:800;color:#C8FF00',txt:data.fuel.liters+' л'}));
      l.appendChild(mk('div',{css:'font-size:11px;color:#949AB5',txt:'Расход топлива'}));
      grid.appendChild(l);
    }
    if (data.fuel.cost) {
      const c = mk('div',{css:'background:rgba(255,255,255,.04);border-radius:10px;padding:12px'});
      c.appendChild(mk('div',{css:'font-family:Syne,sans-serif;font-size:20px;font-weight:800;color:#C8FF00',txt:data.fuel.cost.toLocaleString('ru')+'₽'}));
      c.appendChild(mk('div',{css:'font-size:11px;color:#949AB5',txt:'Стоимость топлива'}));
      grid.appendChild(c);
    }
    fu.appendChild(grid);
    if (data.fuel.comment) fu.appendChild(mk('div',{css:'font-size:12px;color:#949AB5',txt:data.fuel.comment}));
    box.appendChild(fu);
  }

  // Кнопка карты
  if (from && to) {
    const mapBtn = on(mk('button',{css:'width:100%;padding:12px;border:1px solid rgba(255,255,255,.1);border-radius:12px;background:#1E2230;color:#F0F2F8;font-family:Syne,sans-serif;font-size:13px;font-weight:600;cursor:pointer;margin-bottom:8px',txt:'🗺️ Открыть маршрут в Яндекс Картах'}),'click',function(){
      var url = 'https://yandex.ru/maps/?rtext='+encodeURIComponent(from)+'~'+encodeURIComponent(to)+'&rtt=auto';
