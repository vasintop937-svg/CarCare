...
function save(d) {
  ...
  var promises = (d.cars||[]).map(function(car) {
    ...
    return getCarDoc(car.id).set({
      ...
      updated: Date.now(),
      owners: firebase.firestore.FieldValue.arrayUnion(currentUser.uid),
...
function migrateOldData(callback) {
  ...
  oldDoc.get().then(function(snap) {
    if (!snap.exists || !snap.data().data) { callback(false); return; }
    try {
      var old = JSON.parse(snap.data().data);
      if (!old.cars || old.cars.length === 0) { callback(false); return; }
      // Save in new format
      D = old;
      save(old);
      // Delete old document after migration
      setTimeout(function(){ oldDoc.delete(); }, 3000);
      callback(true);
  ...
  db.collection('shared_cars').where('inviteCode','==',normalCode).get()
    .then(function(snap) {
      ...
      carDoc.ref.update({
        ...
        getUserDoc().set({
          carIds: firebase.firestore.FieldValue.arrayUnion(carId)
...
function leaveCar(carId, callback) {
  if (!currentUser) return;
  Promise.all([
    getCarDoc(carId).update({
      owners: firebase.firestore.FieldValue.arrayRemove(currentUser.uid)
    }),
    getUserDoc().update({
      carIds: firebase.firestore.FieldValue.arrayRemove(carId)
    })
